...
export const BRAIN_BUILD_STAGE_ROADMAP_GROUPS = [
  ...
  },
  {
    ...
    stages: [
      ...
      roadmapStage('דרך לתפעול', 19, 'הרחבת ראיות סריקה אמיתיות', 'next', null, null, 'Stage 19A נרשם כתצוגת מטא־דאטה סריקות סטטית על בסיס SCANNED_INTAKE_STATIC_SNAPSHOT בלבד. Stage 19B נרשם ככלי סיווג מטא־דאטה סטטי. Stage 19C מוצג כתצוגת סיווג אפשרי לפי מטא־דאטה בלבד. Stage 19D מוצג כשער אישור אלדד סטטי המחובר ל־metadata-review-gate-helper, ללא שמירה וללא פעולה תפעולית. Stage 19 הרחב עדיין ממתין לאישור מפורש.', ['Stage 19A metadata-only scan intake preview נרשם כתצוגה סטטית בלבד', 'Stage 19B metadata classification helper נרשם ככלי סטטי בלבד', 'Stage 19C classification preview מוצג על בסיס SCANNED_INTAKE_STATIC_SNAPSHOT וה-helper בלבד', 'Stage 19D Eldad review gate preview מחובר ל־metadata-review-gate-helper כתצוגה סטטית בלבד ללא persistence'], ['Stage 19 הרחב לא נפתח', 'אין קריאת תיקייה חיה', 'אין OCR', 'אין קריאת תוכן מקור', 'אין אימות מקור', 'אין confidence גבוה', 'אין שמירה או שער אישור אמיתי'], ['אין קריאת קבצים', 'אין OCR', 'אין יצירת Matter/WorkItem/DocumentRef', 'אין persistence'], AGENT_A_NO_ETA, 'Stage 19A/19B/19C/19D קיימים כשכבות מטא־דאטה ו-review סטטיות בלבד, ללא שמירה; Stage 19 הרחב עדיין ממתין לאישור מפורש.'),
