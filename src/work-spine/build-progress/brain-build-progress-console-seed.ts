...
export const BRAIN_BUILD_LATEST_CHANGE_SUMMARY = {
  title: 'Stage 22A — Visual Architecture Map',
  relatedCommit: '1ae2c22',
  whereToSee: BRAIN_BUILD_PROGRESS_ROUTE,
  whatChanged:
    'נוספה מפת ארכיטקטורה חזותית סטטית של ממשק המוח: 12 visual surfaces בתוך 7 architecture buckets.',
  proofOfLife:
    'מה מוצג במסך: Stage 22A — Visual Architecture Map נרשם כשכבת static planning only. המפה כוללת 12 visual surfaces וכל 7 architecture buckets: Command Center, Work Desk, Process Library, Knowledge Center, Client Workspace, Products & Systems, Internal Dev Tools.',
  stillBlocked: [
    '18D remains HOLD',
    ...
    'Stage 22A is a static visual architecture planning layer only',
    'visual reorg implementation still blocked pending Eldad approval',
    'אין runtime agents',
    'אין workflow execution',
    'אין שינוי Sidebar/Layout/Dashboard',
    'אין שינוי routes',
    'אין שינוי neurons.ts',
    'אין UI/runtime/navigation changes',
    'אין provider',
  ...
  nextSafeStep: 'לבקש אישור אלדד נפרד לפני כל יישום visual reorg בפועל.',
  safetyStatus: 'Stage 22A Visual Architecture Map — static planning only, implementationAllowed:false',
...
export const BRAIN_BUILD_PROGRESS_ITEMS: readonly BrainBuildProgressItem[] = [
  ...
  progressItem(
    ...
    'Stage 21D — Agent ↔ Process Assignment Map',
    'knowledge',
    'knowledge_inventory',
    '9e42b19',
    BRAIN_BUILD_PROGRESS_ROUTE,
    'פתיחת /internal/brain-build-progress לאחר Stage 21D',
    'מה מוצג במסך: 13 static assignments מקשרים 30 internal agents אל 13 process blueprints. static preview only, operationalExecution:false, canRun:false.',
    'built_and_visible',
    'visible_static_preview',
    'preview_only',
    'נוספה מפת שיוך סטטית בין סוכנים פנימיים לבין תהליכי Blueprint מקצועיים.',
    'אלדד רואה ש־Stage 21D מגדיר מי משויך למה כתכנון סטטי בלבד, ללא הפעלת סוכנים או תהליכים.',
    'לא ליצור runtime agents, workflows או UI/navigation changes בלי Gate נפרד.',
    ['אין runtime agents', 'אין workflow execution', 'אין UI/navigation changes', 'אין Sidebar/Layout/Dashboard changes', 'אין routes changes', 'אין neurons.ts changes', 'אין provider', 'אין OCR', 'אין content read', 'אין WorkItem', 'אין Matter', 'אין DocumentRef', 'Stage 20 remains blocked'],
    'Codex ראשי',
  ),
  progressItem(
    'progress-stage-21e-sidebar-separation-plan-v1',
    'Stage 21E — תוכנית הפרדת Sidebar / Process Library',
    'knowledge',
    'knowledge_inventory',
    'e77f4d1',
    BRAIN_BUILD_PROGRESS_ROUTE,
    'פתיחת /internal/brain-build-progress לאחר Stage 21E',
    'מה מוצג במסך: 23 planning rows מפרידות בין Professional Process Blueprints לבין אזורי Sidebar תפעוליים: 13 movesToProcessLibrary, 8 staysInSidebar, 2 needsEldadDecision. static planning only, implementationAllowed:false, requiresEldadApproval:true.',
    'built_and_visible',
    'visible_static_preview',
    'preview_only',
    'נוספה תוכנית הפרדה סטטית עבור פריטי Sidebar ותהליכי Process Library עתידיים.',
    'אלדד רואה ש־Stage 21E ממפה מה נשאר ב־Sidebar, מה עובר ל־Process Library, ומה דורש החלטת אלדד.',
    'לא לבצע שינויי Sidebar, Layout, Dashboard או routes בלי Gate יישום נפרד.',
    ['אין UI/navigation changes', 'אין Sidebar/Layout/Dashboard changes', 'אין routes changes', 'אין neurons.ts changes', 'אין שינוי sidebarNav.ts', 'אין שינוי Layout.tsx', 'אין שינוי dashboard files', 'אין runtime behavior', 'אין WorkItem', 'אין Matter', 'אין DocumentRef', 'Stage 20 remains blocked'],
    'Codex ראשי',
  ),
  progressItem(
    'progress-stage-22a-visual-architecture-map-v1',
    'Stage 22A — Visual Architecture Map',
    'visual_surface',
    'surface_inventory',
    '1ae2c22',
    BRAIN_BUILD_PROGRESS_ROUTE,
    'פתיחת /internal/brain-build-progress לאחר Stage 22A',
    'מה מוצג במסך: Stage 22A — Visual Architecture Map נרשם כשכבת static planning only עם 12 visual surfaces ו־7 architecture buckets. אין UI/runtime/navigation changes.',
    'built_and_visible',
    'visible_static_preview',
    'preview_only',
    'נוספה מפת ארכיטקטורה חזותית סטטית שמפרידה בין Command Center, Work Desk, Process Library, Knowledge Center, Client Workspace, Products & Systems, Internal Dev Tools.',
    'אלדד רואה ש־Stage 22A ממפה את תפקיד משטחי המוח החזותיים בלי לבצע visual reorg.',
    'לא לבצע visual reorg, שינוי Layout, שינוי Sidebar, שינוי Dashboard או שינוי routes בלי Gate יישום נפרד.',
    ['אין UI/runtime/navigation changes', 'אין שינוי Layout.tsx', 'אין שינוי sidebar/routes/dashboard/CEO/UI files', 'אין שינוי neurons.ts', 'visual reorg implementation still blocked pending Eldad approval', 'אין runtime behavior', 'אין WorkItem', 'אין Matter', 'אין DocumentRef', 'Stage 20 remains blocked'],
    'Codex ראשי',
  ),
