...
CREATE TABLE IF NOT EXISTS clients (
  ...
  status TEXT NOT NULL DEFAULT 'פעיל' CHECK (status IN ('פעיל', 'הושלם', 'מוקפא')),
...
-- ═══════════════════════════════════════════════════════
-- 🤖 AGI Infrastructure (Phase 1 — Agent Tool Calling + Memory)
-- ═══════════════════════════════════════════════════════

-- Agent Run History — every agent step execution is recorded
...
-- Tool Execution Results — every tool call an agent makes
...
-- Agent Notes — findings, decisions, recommendations saved by agents
CREATE TABLE IF NOT EXISTS agent_notes (
  ...
  process_id TEXT,
  client_id TEXT,
  note_type TEXT DEFAULT 'finding' CHECK (note_type IN ('finding', 'decision', 'recommendation', 'warning')),
  content TEXT NOT NULL,
  created_at TIMESTAMPTZ DEFAULT NOW()
);

CREATE INDEX IF NOT EXISTS idx_agent_notes_mission ON agent_notes(mission_id);
CREATE INDEX IF NOT EXISTS idx_agent_notes_agent ON agent_notes(agent_id);
CREATE INDEX IF NOT EXISTS idx_agent_notes_process ON agent_notes(process_id);
CREATE INDEX IF NOT EXISTS idx_agent_notes_type ON agent_notes(note_type);

-- Document Drafts — documents generated by agents
CREATE TABLE IF NOT EXISTS document_drafts (
  id TEXT PRIMARY KEY,
  title TEXT NOT NULL,
  doc_type TEXT NOT NULL CHECK (doc_type IN ('letter', 'report', 'analysis', 'opinion', 'memo')),
  content TEXT NOT NULL,
  process_id TEXT,
...
CREATE INDEX IF NOT EXISTS idx_document_drafts_type ON document_drafts(doc_type);

CREATE OR REPLACE TRIGGER document_drafts_updated_at
